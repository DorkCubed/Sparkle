...
def setup_logging(log_dir="logs"):
    ...
    log_file = f'{log_dir}/eval_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'
    logging.basicConfig(
        level=logging.INFO,
...
class PacketLevelEvaluator:
    def __init__(self, packet_embedding, packet_encoder, flow_embedding, flow_encoder, 
                 eval_manifest_path, max_samples=None):
        self.config = Config()
    ...
    def safe_prepare(self, tensor, name):
        ...
        try:
            ...
            self.skipped += 1
            raise RuntimeError(f"Failed to move {name} to device {self.device}: {e}") from e
        ...
        with torch.no_grad():
            ...
            for i, (packet_sequences, field_position, header_position, entry) in enumerate(progress_bar):
                ...
                try:
                    ...
                    packet_sequences = self.safe_prepare(packet_sequences, "packet_sequences")
                    field_position = self.safe_prepare(field_position, "field_position")
                    header_position = self.safe_prepare(header_position, "header_position")
            ...
            try:
                if self.all_packet_encodings and self.previous_entry and self.previous_entry.get("packet"):
                    mpm_loss, num_chunks = self.process_encodings(
                        self.all_packet_encodings, self.previous_entry
                    )
                    if mpm_loss is not None:
    ...
    def _compute_metrics(self):
        ...
        metrics = {
            ...
            "avg_packet_loss": (self.total_mlm_loss + self.total_sfbo_loss) / total_processed,
            "avg_mpm_loss": self.total_mpm_loss / max(self.num_batches, 1) if self.num_batches > 0 else 0.0,
            "total_loss": self.total_loss + self.total_mpm_loss,
            "num_samples": self.num_samples,
            "num_batches": self.num_batches,
            "skipped": self.skipped,
            "perplexity": torch.exp(torch.tensor((self.total_loss + self.total_mpm_loss) / max(total_processed + self.num_batches, 1))).item()
        }
...
def load_model_from_checkpoint(checkpoint_path, config, vocab, device):
    ...
    logger = logging.getLogger(__name__)
    logger.info(f"Loaded checkpoint from {checkpoint_path} (epoch {checkpoint.get('epoch', 'unknown') + 1})")
...
def run_evaluation(
    checkpoint_path="checkpoints/checkpoint_1.pth",
    eval_manifest_path="manifest/eval_manifest.json",
    max_samples=None,
    ...
    packet_embedding, packet_encoder, flow_embedding, flow_encoder = load_model_from_checkpoint(
    ...
    if output_json_path is None:
        ...
        output_json_path = f"logs/eval_results_{timestamp}.json"
