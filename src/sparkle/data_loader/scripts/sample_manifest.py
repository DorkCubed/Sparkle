...
class S3SampleDataFetcher:
    ...
    def sample_manifest(self, manifest, k=1000):
        if len(manifest) < k:
            raise ValueError(
                f"Manifest contains only {len(manifest)} entries but k={k}."
            )
...
if __name__ == "__main__":
    ...
    original_manifest_path = os.path.join(project_dir, "manifest", "manifest.json")

    sample_length = 50

    sampled_manifest_path = os.path.join(
        project_dir, "manifest", f"manifest_{sample_length}.json"
    )
